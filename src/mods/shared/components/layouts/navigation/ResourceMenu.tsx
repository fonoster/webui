...
export const ResourceMenu = () => {
  ...
  return (
    <Menu as="div" className="z-10 absolute bottom-8 right-8 flex-shrink-0">
      ...
      >
        <Menu.Items className="z-10 origin-bottom-right absolute overflow-y-auto max-h-[32rem] bottom-20 right-0 mt-2 w-80 rounded-md shadow-lg py-1 bg-gray-700 ring-1 ring-black ring-opacity-5 focus:outline-none">
          {nav.map(item => (
            <Menu.Item key={item.name}>
              {({ active }) => (
                <button
                  className={classes(
                    active ? 'bg-gray-600' : '',
                    'flex w-full block px-4 py-2 text-sm text-gray-300 text-left'
                  )}
